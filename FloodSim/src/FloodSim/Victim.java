...
public class Victim implements Steppable, Valuable, java.io.Serializable{
    ...
    public Victim(int age, int sex, int weight, int height, Family hh, GroundControl home, GroundControl position, MersenneTwisterFast random, Continuous2D allVictims){
    	this.setAge(age);
        this.setSex(sex);
        this.setFamily(hh);
        this.setHome(home);
    ...
    public String getX() {
        return Integer.toString(position.getX());
    }
    
    public String getY() {
        return Integer.toString(position.getY());
    }
    ...
    public int setVictimType(){
    	...
    	else if (this.getAge() >= 10 && this.getAge() <= 25){
	...
	private double healthActivityWeight() {
        double wHealthC;
        int CurrHealthStatus = this.getHealthStatus();
        
        if (CurrHealthStatus == 2 && this.getIsrecieveTreatment() == false) { // weight high
            wHealthC = 0.7 + 0.3 * randomN.nextDouble();        
        }
        else if (this.getIsSick() || CurrHealthStatus == 3){
    ...
    private double shelterActivityWeight() {
    	double wShelter;
    	...
    	if (this.VictimType == 1){
    		if (this.getWaterLevel() >= f.params.global.getMaxWaterThreadholdOldster()){
    			...
    			wShelter = randomN.nextDouble()*(0.1 +  0.2 * randomN.nextDouble());
    		}
    	}
...
*/    
    ...
    public int actSelect() {
        ...
        if (!isDayTime) {
            double wHealth = 0;
            if(this.getHealthStatus() == 2 || this.getHealthStatus() == 3 ){
                wHealth = healthActivityWeight();
            }
            else{ // healthy or dead : dont go HEALTH_CENTER
                wHealth = 0;
            }
            ...
            activityPriortyWeight[3] = shelterActivityWeight();
        ...
        public void recieveTreatment(GroundControl fu, Flood f) {
            // based on the capacity of the Health Center <<--- ToDo : check FullCapacty before check health status
            // be infected or Sick + Treat
            if (this.getHealthStatus() != 1 && fu.getFacility().isFullCapacity(fu,f) == false) {
                ...
                if(randomN.nextDouble() < f.params.global.getprobabilityOfEffectiveNessofmedicine()){
	                ...
	                //this.setCountIgnoreTreat(this.getCountIgnoreTreat() - 1); <<-- check is zero
                }     
            }
         // be infected or Sick +  No Treat
            else if (this.getHealthStatus() != 1 && this.getHealthStatus() != 4 && fu.getFacility().isFullCapacity(fu,f) == true){
            	this.setCountIgnoreTreat(this.getCountIgnoreTreat() + 1);
            	this.setIsrecieveTreatment(false);
            	//this.setHealthStatus(this.getHealthStatus() + 1);
            }
        }
