...
public class RegionGenerator {
	 ...
	 // add households
	 private static void addAllVictims(int age, int sex, int height, int weight, Family hh, MersenneTwisterFast random,Flood f) {
		...
		// final Color Agent2 = new Color(255, 246, 143);
		 ...
		 Victim newVictim = new Victim(age, sex, height, weight, hh, hh.getRegionLocation(), hh.getRegionLocation(), random, f.allVictims);
	     hh.addMembers(newVictim);
	     hh.getRegionLocation().addVictim(newVictim);
	     
	     int rand = f.random.nextInt(100); //start program no Health status Dead(4)
	     if (rand < 50){ //healthy 50%
	    	 newVictim.setHealthStatus(1);
	     }
	     else if (rand > 50 && rand < 95){ //Infected 45%
	            ...
	            if (tot != 0 && counter <= totalVictim ) {
	                ...
	                for (int i = 0; i < tot; i++) {
	                    ...
	                    //initial height & weight
	                     int height = 160;
	                     int weight = 50;
	                     addAllVictims(age,sex,height,weight, hh, random,f);
	                }
	            }
	        }
	 }
	            ...
	            for (int i = 0; i < size; i++) {
	                ...
	                if (n == null) { // found something new! Mark it and reproduce
	                    ...
	                    for (Object o : neighbors) {
	                        ...
	                        if (l == c.location || c.location.getWaterID() == 1) {
	                            continue;
	                        }
